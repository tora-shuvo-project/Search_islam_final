import 'package:search_islam/helper/hijri_calendar/digits_converter.dart';
import 'package:search_islam/helper/hijri_calendar/hijri_array.dart';
...
class HijriCalendar {
  ...
  format(year, month, day, format) {
    ...
    if (newFormat.contains("DDDD")) {
      newFormat = newFormat.replaceFirst("DDDD", "${_local[language]['days'][wkDay ?? wekDay()]}");

      // Fri
    } else if (newFormat.contains("DD")) {
      newFormat = newFormat.replaceFirst("DD", "${_local[language]['short_days'][wkDay ?? wekDay()]}");
    }
  ...
  bool isBefore(int year, int month, int day) {
    return hijriToGregorian(hYear, hMonth, hDay).millisecondsSinceEpoch < hijriToGregorian(year, month, day).millisecondsSinceEpoch;
  }
